...
pkill -x Regatta || true
sleep 0.2
APP_PATH="$(
  find "$HOME/Library/Developer/Xcode/DerivedData" -path "*/Build/Products/Release/Regatta.app" -print0 \
...
if [[ -z "${APP_PATH}" ]]; then
  echo "Regatta.app not found in DerivedData" >&2
  exit 1
fi
...
APP_PROCESS_PATH="${APP_PATH}/Contents/MacOS/Regatta"
ATTEMPT=0
MAX_ATTEMPTS=20
