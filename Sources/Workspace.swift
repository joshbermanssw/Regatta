...
final class Workspace: Identifiable, ObservableObject {
    ...
    private static func shouldRunPromptedSurfaceResumeOnMain(_ binding: SurfaceResumeBindingSnapshot) -> Bool {
        ...
        alert.informativeText = String(
            format: String(
                ...
                defaultValue: "Regatta is restoring a terminal with this resume command:\n\n%@\n\nWorking directory: %@"
            ),
            binding.command,
    ...
    private static func remoteDisconnectPlaceholderScript(target: String, reconnectCommand: String?) -> String {
        ...
        let endedLineFormat = String(
            ...
            defaultValue: "[Regatta] remote session disconnected: %s"
        )
        let reconnectLine = String(
            localized: "remote.disconnectBanner.reconnectHint",
            defaultValue: "[Regatta] Press Enter to reconnect. This terminal will stay disconnected until then."
        )
        let reconnectUnavailableLine = String(
            localized: "remote.disconnectBanner.reconnectUnavailableHint",
            defaultValue: "[Regatta] Reconnect this workspace from the sidebar or by running the original Regatta remote command again."
        )
    ...
    private func showMoveTabFailureAlert() {
        ...
        failure.informativeText = String(localized: "alert.moveTab.failed.message", defaultValue: "Regatta could not move this tab to the selected destination.")
