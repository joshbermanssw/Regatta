    ...
    private func confirmKillProcess(row: CmuxTaskManagerRow, processIds: [Int]) -> Bool {
        ...
        if processIds.count == 1, let processId = processIds.first {
            ...
            alert.informativeText = String(format: String(
                ...
                defaultValue: "Ask %@ (PID %lld) to terminate gracefully. Regatta will force-kill it if it is still running after a short grace period."
            ...
            alert.informativeText = String(format: String(
                ...
                defaultValue: "Ask %lld processes to terminate gracefully. Regatta will force-kill remaining processes after a short grace period. PIDs: %@."
