    ...
    static var appLocalized: RemoteDaemonStrings {
        RemoteDaemonStrings(
            missingPersistentPTYCapability: String(
                ...
                defaultValue: "remote daemon does not support persistent SSH PTY sessions; reconnect the remote workspace to update Regatta"
            ),
            missingRequiredFunctionality: String(
                localized: "remoteDaemon.error.missingRequiredFunctionality",
                defaultValue: "remote daemon is missing required functionality; reconnect the remote workspace to update Regatta"
            )
        )
    }
}
